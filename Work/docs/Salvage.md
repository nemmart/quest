...
| P5 | **`WPSH 3,3` / `LDAFP 3` / … / `WPOP 3,3`** around a frame reference when ac0–ac2 are all live and ac3 holds a base. | FIRE.1 7016A3EA..EE; INIT_OBJ_TBL 7016DF8F (block@7016DF68) | **Verified** |
| P6 | **The frame pointer is displaced and re-materialised by `LDAFP` into ac2 or ac3**; FIRE.1 has 3× `LDAFP 2` and 3× `LDAFP 3`; DIED holds the frame in ac2 for ~10 blocks (22 `wp(ac2,8)` refs) while ac3 holds a record base. | FIRE.1 block@7016A3C7; DIED 70166376..3B3 | **Verified** (FIRE.1) / Reported (DIED counts) |
...
| R5, R41, R41′ | base-register class {ac2,ac3}; the SD_PTR census (M1) survives, the class rule does not | yes (M1 is the census) |
