...
layout columns are PROVISIONAL (ruling, Sep 6 2026): capacity 4096 bytes,
rows 0x1000 words apart (extent = 1 + cap/2 words plus the closed right
end, so 0x800-word spacing would violate I1). P33-B replaces the capacity
column from lowering.
...
ARENA_STRIDE = 0x1000      # words
CAPACITY = 4096            # bytes, provisional
...
def main():
    ...
    out.append(f'# arena layout PROVISIONAL: capacity {CAPACITY} bytes per row, rows 0x{ARENA_STRIDE:X} words apart '
               f'from 0x{ARENA_BASE:08X} — P33-B replaces the capacity column from lowering')
    out.append('# row <id> <block> <routine> arena=<word addr> cap=<bytes> first=<pc> last=<pc> nclaims=<n>')
    ...
    for i, (blk, routine, cl, stasp) in enumerate(groups, 1):
        arena = ARENA_BASE + (i - 1) * ARENA_STRIDE
        out.append(f'row {i} {blk:08X} {routine} arena={arena:08X} cap={CAPACITY} first={cl[0]:08X} last={cl[-1]:08X} nclaims={len(cl)}')
