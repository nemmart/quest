...
set -eu
# ---------------------------------------------------------------------------
# P53 CLEARED THE EXPECTED-RED GATE (P52/q001 R10, granted in P52/a002).
# Both causes P52 named are gone: lower_c.py emits `ir 8` in the variable form,
# and the SUB() tokens in cases/{sub_traps,shortcircuit}.c are renamed.
# Set QUEST_DIFFTEST_EXPECT_RED=1 to re-gate if this ever needs parking again.
# ---------------------------------------------------------------------------
if [ "${QUEST_DIFFTEST_EXPECT_RED:-0}" = "1" ]; then
  echo "run_lowerc_difftest.sh: SKIPPED (QUEST_DIFFTEST_EXPECT_RED=1)"
  exit 0
fi
cd "$(dirname "$0")/.."
SEEDS="${1:-100}"
...
echo
echo "=== $SEEDS seeds x 4 classes (D is the byte class, P53) ==="
python3 "$DT" --seeds "$SEEDS" --seed0 "$SEED0" --classes ABCD --rig "$RIGDIR/lowerc_rig"
...
for M in u16_unsigned no_sign_extend eager_bool shift_logical cmp_unsigned abs_argtype byte_as_word bp_scaled; do
  if python3 "$DT" --seeds 6 --classes ABCD --mutate "$M" --expect-red --rig "$RIGDIR/lowerc_rig" \
       >/tmp/p48/teeth.$M.txt 2>&1; then
    echo "  caught: $M ($(grep -c '^!! ' /tmp/p48/teeth.$M.txt) of 24 programs)"
  else
...
echo
echo
echo "=== quest.assumptions (P53) ==="
# Every row checked three ways, over the IR this run just produced.
python3 ../compiler/check_assumptions.py $(find /tmp/p48-difftest -name 'prog.ir' 2>/dev/null | head -40) /tmp/p48/us.ir
