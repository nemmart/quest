    ...
    for(int i = 0; i < 4; i++) {
      ...
      if(v.kind == hw::Mapper::Kind::MISMATCH && hw::Mapper::is_arena(v.clone_word)) {
        ...
        if(row == nullptr)
          ...
          printf("  ac%d: clone value %08X hits UNMAPPED arena row s@%08X.%u arena=%08X — no master temp for it now\n", i,
                 static_cast<uint32_t>(clone->machine->ac[i]), row->block, row->claim, row->arena_addr);
        else
          printf("  ac%d: clone value %08X hits mapped arena row s@%08X.%u arena=%08X master=%08X wfp=%08X, maps to %08X != master %08X\n", i,
