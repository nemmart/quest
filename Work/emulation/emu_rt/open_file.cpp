...
uint32_t open_file(Machine& machine) {
  ...
  if(argc!=2) {
    return machine.symbols->address_for_name("?OPEN_FILE");
  }
  ...
  try {
    ...
    int32_t channel=0;
    rt::open_file_2(ctx, channel, filename);
    // Write channel number back to *arg1
