#!/bin/bash
# Task 034 — PARALLEL battery template (user ruling Aug 29: kill the
# 30+ minute serial runs). Successor to the hold/031 hardened SERIAL
# template; leg internals unchanged from 032/033.
#
# Parallelism model:
#  - Legs are already isolated (own port, own /tmp/run034-$tag scratch,
...
git checkout --quiet origin/main -- Work Disassembled
...
RES=$ROOT/results/034-parallel-battery; mkdir -p $RES
JOBS=${JOBS:-6}
T0=$(date +%s)
for prt in $(seq 8791 8803); do fuser -k -TERM $prt/tcp 2>/dev/null || true; done; sleep 2

sed 's|("127.0.0.1", 8781)|("127.0.0.1", int(__import__("os").environ.get("QUEST_PORT", "8781")))|' \
  $W/docs/Project13/drive.py > /tmp/drive034.py
sed 's|("127.0.0.1", 8781)|("127.0.0.1", int(__import__("os").environ.get("QUEST_PORT", "8781")))|' \
  $W/docs/Project14/drive_patient.py > /tmp/drive_patient034.py

cat > /tmp/blk034.py <<'PYEOF'
import re, sys
...
leg(){ local tag=$1 mode=$2 drv=$3 port=$4 k=$5 floor=$6 want=$7 cfg=$8; shift 8
  local R=/tmp/run034-$tag; rm -rf $R; mkdir -p $R; cp -r $ROOT/QUEST $R/QUEST; cd $R
  ...
  local blk bstat=OK; blk=$(python3 /tmp/blk034.py $R/trace $k $floor) || bstat=FAIL
  local end="clean"
...
launch play      play     /tmp/drive_patient034.py 8795 50 1000  clean,I.STOP   book
launch play-st   play     /tmp/drive034.py         8799 50 1000  clean,I.STOP   stock
launch fo        failopen /tmp/drive034.py         8791 50 1000  clean,I.STOP   book  QUEST_FAIL_OPEN=USER_DATA_FILE
launch m         m        /tmp/drive034.py         8792 50 1000  I.STOP         book
launch inj       play     /tmp/drive034.py         8793 50 1000  FATAL          book  QUEST_INJECT=7016A896:-1:0x2006
launch abort     m        /tmp/drive034.py         8794 50 10    WORLD-ABORT    book  QUEST_TERMINAL=7016871D:ABORT
launch inj3      m        /tmp/drive034.py         8796 50 1000  clean,I.STOP   book  QUEST_INJECT=70176AA7:-1:0x2006:RESUME
launch k1fo      failopen /tmp/drive034.py         8797 1  1000  clean,I.STOP   book  QUEST_FAIL_OPEN=USER_DATA_FILE
launch fo-st     failopen /tmp/drive034.py         8798 50 1000  clean,I.STOP   stock QUEST_FAIL_OPEN=USER_DATA_FILE
launch fo-emu    failopen /tmp/drive034.py         8800 50 1000  clean,I.STOP   emu   QUEST_FAIL_OPEN=USER_DATA_FILE
launch m-emu     m        /tmp/drive034.py         8801 50 1000  I.STOP         emu
launch inj-emu   play     /tmp/drive034.py         8802 50 1000  FATAL          emu   QUEST_INJECT=7016A896:-1:0x2006
launch abort-emu m        /tmp/drive034.py         8803 50 10    WORLD-ABORT    emu   QUEST_TERMINAL=7016871D:ABORT
wait || true
...
echo "== Task 034 parallel battery (JOBS=$JOBS) ==" | tee -a $RES/verdicts.txt
