#!/bin/bash
# Task 052 — P46 FINAL (Stages A–D landed on branch p46-ir7): the 047b battery verbatim
# (as task 050) on the FINAL branch — the Stage C/D loader (v declarations, symbolic blocks,
# the 0x76/0x77 placement, the width tripwire) against the UNCHANGED ir 7 book. Bar as 050:
# 16/16 legs (+forced), 0 div on every non-forced leg, embeds 557/2436, s@ census 0/236 0/236
# 0/57, ir 7 headers — plus the new precondition: tests/run_vform_selftest.sh GREEN with its
# broken-allocator teeth RED (the v-form program loads, places and executes through
# Machine::run_steps). Also counts `0x76 space` mapping lines across the IR legs (want 0: the
# book declares no v, so the Stage C/D loader must map nothing new).
# Known: 050 and 047b both exit 1 on 047b-era `want` lines that predate P33 (task 051 owns the
# refresh; docs/Project46/evidence/task050_exit1_preexisting.txt). This task carries the same
# lines unchanged so the two results stay comparable; read verdicts.txt, not only the marker.
set -eu
exec 9>/tmp/quest-parallel-battery.lock
flock -n 9 || { echo "another battery attempt is still running; refusing overlap"; exit 1; }
cd "$(dirname "$0")/.."
ROOT=$(pwd)
SRC=$(bin/task_source.sh p46-ir7 052-p46-final); W=$SRC/Work
...
RES=$ROOT/results/052-p46-final; mkdir -p $RES
...
( cd $W/emulation && bash tests/run_helpers_selftest.sh ) > /tmp/selftest052 2>&1 || { echo "helpers selftest RED"; cat /tmp/selftest052; exit 1; }
( cd $W/emulation && bash tests/run_strings_selftest.sh ) > /tmp/strings052 2>&1 || { echo "strings selftest RED"; cat /tmp/strings052; exit 1; }
( cd $W/emulation && bash tests/run_strhooks_selftest.sh ) > /tmp/strhooks052 2>&1 || { echo "strhooks selftest RED"; cat /tmp/strhooks052; exit 1; }
grep -q "teeth confirmed" /tmp/strhooks052 || { echo "strhooks teeth NOT confirmed"; exit 1; }
( cd $W/emulation && bash tests/run_vform_selftest.sh ) > /tmp/vform052 2>&1 || { echo "vform selftest RED"; cat /tmp/vform052; exit 1; }
grep -q "teeth confirmed" /tmp/vform052 || { echo "vform teeth NOT confirmed"; exit 1; }
JOBS=${JOBS:-3}
T0=$(date +%s)
...
sed 's|("127.0.0.1", 8781)|("127.0.0.1", int(__import__("os").environ.get("QUEST_PORT", "8781")))|' \
  $W/docs/Project13/drive.py > /tmp/drive052.py
sed 's|("127.0.0.1", 8781)|("127.0.0.1", int(__import__("os").environ.get("QUEST_PORT", "8781")))|' \
  $W/docs/Project14/drive_patient.py > /tmp/drive_patient052.py

cat > /tmp/blk052.py <<'PYEOF'
import re, sys
...
leg(){ local tag=$1 mode=$2 drv=$3 port=$4 k=$5 floor=$6 want=$7 cfg=$8; shift 8
  local R=/tmp/run052-$tag; rm -rf $R; mkdir -p $R; cp -r $ROOT/QUEST $R/QUEST; cd $R
  ...
  local blk bstat=OK; blk=$(python3 /tmp/blk052.py $R/trace $k $floor) || bstat=FAIL
  local end="clean"
...
launch play      play     /tmp/drive_patient052.py 8895 50 1000  clean,I.STOP   book "${STR[@]}"
launch play-st   play     /tmp/drive052.py         8899 50 1000  clean,I.STOP   stock "${STR[@]}"
launch fo        failopen /tmp/drive052.py         8891 50 1000  clean,I.STOP   book "${STR[@]}"  QUEST_FAIL_OPEN=USER_DATA_FILE
launch m         m        /tmp/drive052.py         8892 50 1000  I.STOP         book "${STR[@]}"
launch inj       play     /tmp/drive052.py         8893 50 1000  FATAL          book "${STR[@]}"  QUEST_INJECT=7016A896:-1:0x2006
launch abort     m        /tmp/drive052.py         8894 50 10    WORLD-ABORT    book "${STR[@]}" QUEST_TERMINAL=7016871D:ABORT
launch inj3      m        /tmp/drive052.py         8896 50 1000  clean,I.STOP   book "${STR[@]}"  QUEST_INJECT=70176AA7:-1:0x2006:RESUME
launch k1fo      failopen /tmp/drive052.py         8897 1  1000  clean,I.STOP   book "${STR[@]}"  QUEST_FAIL_OPEN=USER_DATA_FILE
launch derr      failopen /tmp/drive052.py         8904 1  1000  WORLD-ABORT    book "${STR[@]}"  QUEST_FAIL_OPEN=USER_DATA_FILE QUEST_POKE=7015C48B:0:11
launch derr-emu  failopen /tmp/drive052.py         8905 1  1000  WORLD-ABORT    emu "${STR[@]}"   QUEST_FAIL_OPEN=USER_DATA_FILE QUEST_POKE=7015C48B:0:11
launch fo-st     failopen /tmp/drive052.py         8898 50 1000  clean,I.STOP   stock "${STR[@]}" QUEST_FAIL_OPEN=USER_DATA_FILE
launch fo-emu    failopen /tmp/drive052.py         8900 50 1000  clean,I.STOP   emu "${STR[@]}"   QUEST_FAIL_OPEN=USER_DATA_FILE
launch m-emu     m        /tmp/drive052.py         8901 50 1000  I.STOP         emu "${STR[@]}"
launch inj-emu   play     /tmp/drive052.py         8902 50 1000  FATAL          emu "${STR[@]}"   QUEST_INJECT=7016A896:-1:0x2006
launch abort-emu m        /tmp/drive052.py         8903 50 10    WORLD-ABORT    emu "${STR[@]}"   QUEST_TERMINAL=7016871D:ABORT
launch forced    failopen /tmp/drive052.py         8907 1  10    DIVERGENCE     book "${STR[@]}" QUEST_FAIL_OPEN=USER_DATA_FILE QUEST_POKE=7015C48B:2:0x75000000:CLONE
wait || true
...
echo "== Task 052 P46 FINAL: ir 7 loader (v/symbolic blocks) on the unchanged book (047b battery verbatim; checker ON) (JOBS=$JOBS) ==" | tee -a $RES/verdicts.txt
...
grep -h "^strings.*QUEST1 claim" /tmp/run052-*/trace 2>/dev/null | sed 's/.*block=\([0-9A-F]*\).*/\1/' | sort | uniq -c | sort -rn | tee -a $RES/verdicts.txt
...
tail -1 /tmp/selftest052 | tee -a $RES/verdicts.txt; tail -1 /tmp/strings052 | tee -a $RES/verdicts.txt; tail -1 /tmp/strhooks052 | tee -a $RES/verdicts.txt; tail -1 /tmp/vform052 | tee -a $RES/verdicts.txt
...
[ "$FAILS" = "0" ] || { echo "TASK 052 RED ($FAILS fails)"; exit 1; }
echo "TASK 052 GREEN"
