#!/bin/bash
# Task 035 — P25 byte addressing + closed call ledger (566/566).
# COPY of the 034 parallel template of record (13 legs, JOBS pool;
# per the P25 prompt: do NOT use 031/032's serial shape); pointed at
# the p25-byte-addressing branch artifacts, with the newly-lowered-
# site coverage report appended to the verdicts alongside the carry
# line.
#
# Parallelism model:
#  - Legs are already isolated (own port, own /tmp/run035-$tag scratch,
...
git fetch --quiet origin p25-byte-addressing
git checkout --quiet origin/p25-byte-addressing -- Work Disassembled
...
RES=$ROOT/results/035-p25-byte-battery; mkdir -p $RES
JOBS=${JOBS:-6}
T0=$(date +%s)
for prt in $(seq 8811 8823); do fuser -k -TERM $prt/tcp 2>/dev/null || true; done; sleep 2

sed 's|("127.0.0.1", 8781)|("127.0.0.1", int(__import__("os").environ.get("QUEST_PORT", "8781")))|' \
  $W/docs/Project13/drive.py > /tmp/drive035.py
sed 's|("127.0.0.1", 8781)|("127.0.0.1", int(__import__("os").environ.get("QUEST_PORT", "8781")))|' \
  $W/docs/Project14/drive_patient.py > /tmp/drive_patient035.py

cat > /tmp/blk035.py <<'PYEOF'
import re, sys
...
leg(){ local tag=$1 mode=$2 drv=$3 port=$4 k=$5 floor=$6 want=$7 cfg=$8; shift 8
  local R=/tmp/run035-$tag; rm -rf $R; mkdir -p $R; cp -r $ROOT/QUEST $R/QUEST; cd $R
  ...
  local blk bstat=OK; blk=$(python3 /tmp/blk035.py $R/trace $k $floor) || bstat=FAIL
  local end="clean"
...
launch play      play     /tmp/drive_patient035.py 8815 50 1000  clean,I.STOP   book
launch play-st   play     /tmp/drive035.py         8819 50 1000  clean,I.STOP   stock
launch fo        failopen /tmp/drive035.py         8811 50 1000  clean,I.STOP   book  QUEST_FAIL_OPEN=USER_DATA_FILE
launch m         m        /tmp/drive035.py         8812 50 1000  I.STOP         book
launch inj       play     /tmp/drive035.py         8813 50 1000  FATAL          book  QUEST_INJECT=7016A896:-1:0x2006
launch abort     m        /tmp/drive035.py         8814 50 10    WORLD-ABORT    book  QUEST_TERMINAL=7016871D:ABORT
launch inj3      m        /tmp/drive035.py         8816 50 1000  clean,I.STOP   book  QUEST_INJECT=70176AA7:-1:0x2006:RESUME
launch k1fo      failopen /tmp/drive035.py         8817 1  1000  clean,I.STOP   book  QUEST_FAIL_OPEN=USER_DATA_FILE
launch fo-st     failopen /tmp/drive035.py         8818 50 1000  clean,I.STOP   stock QUEST_FAIL_OPEN=USER_DATA_FILE
launch fo-emu    failopen /tmp/drive035.py         8820 50 1000  clean,I.STOP   emu   QUEST_FAIL_OPEN=USER_DATA_FILE
launch m-emu     m        /tmp/drive035.py         8821 50 1000  I.STOP         emu
launch inj-emu   play     /tmp/drive035.py         8822 50 1000  FATAL          emu   QUEST_INJECT=7016A896:-1:0x2006
launch abort-emu m        /tmp/drive035.py         8823 50 10    WORLD-ABORT    emu   QUEST_TERMINAL=7016871D:ABORT
wait || true
...
echo "== Task 035 P25 byte battery (JOBS=$JOBS) ==" | tee -a $RES/verdicts.txt
...
echo "-- P25 newly-lowered-site block coverage (ByteEA.md; book legs) --" | tee -a $RES/verdicts.txt
echo "LPEFB+parse-fix sites:" | tee -a $RES/verdicts.txt
for b in 7015C2B2 7015C323 7017178A 70171965 70171E7F 70174760 70174788 701747AE \
         701747D4 701747FA 70174828 70177A1A 70177E7A 70177EF4 70177F03 7017867E \
         701787F2 7017D26C 7017D3D5; do
  c=$(grep -h "first execution of block $b" $RES/*.err 2>/dev/null | wc -l)
  printf "%s:%s " $b $c
done | tee -a $RES/verdicts.txt
echo | tee -a $RES/verdicts.txt
echo "WPSH multi-wide sites:" | tee -a $RES/verdicts.txt
for b in 7015BE5F 7015C4FB 70160E9A 70161714 70161861 70161AFB 70161B46 70161B6B \
         70161BC3 70161BE8 70163014 70165127 701670D7 70169B77 7016B81D 7016F259 \
         70171164 70171325 70172642 7017341C 701740C8 70175EB0 70175EE8 7017BE89 \
         7017D7C4; do
  c=$(grep -h "first execution of block $b" $RES/*.err 2>/dev/null | wc -l)
  printf "%s:%s " $b $c
done | tee -a $RES/verdicts.txt
echo | tee -a $RES/verdicts.txt
echo "borrow sites (UPDATE_SCREENS; census-carried if 0 — ByteEA.md §5):" | tee -a $RES/verdicts.txt
for b in 7015F795 7016A3C7; do
  c=$(grep -h "first execution of block $b" $RES/*.err 2>/dev/null | wc -l)
  printf "%s:%s " $b $c
done | tee -a $RES/verdicts.txt
echo | tee -a $RES/verdicts.txt
LIVE=0; TOT=0; LIVEIDS=""
for b in $(grep -oE "^[0-9A-F]{8}" $ROOT/tasks/035-bform-blocks.txt); do
  TOT=$((TOT+1))
  c=$(grep -h "first execution of block $b" $RES/*.err 2>/dev/null | wc -l)
  if [ "$c" != "0" ]; then LIVE=$((LIVE+1)); LIVEIDS="$LIVEIDS $b"; fi
done
echo "B-form (GET_INPUT) blocks live: $LIVE/$TOT —$LIVEIDS" | tee -a $RES/verdicts.txt
