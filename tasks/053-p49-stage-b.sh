...
cd "$(dirname "$0")/.."
ROOT=$(pwd)
SRC=$(bin/task_source.sh p49-playdriver 053-p49-stage-b); W=$SRC/Work
...
RES=$ROOT/results/053-p49-stage-b; mkdir -p $RES
...
( cd $W/emulation && bash tests/run_helpers_selftest.sh ) > /tmp/selftest053 2>&1 || { echo "helpers selftest RED"; cat /tmp/selftest053; exit 1; }
( cd $W/emulation && bash tests/run_strings_selftest.sh ) > /tmp/strings053 2>&1 || { echo "strings selftest RED"; cat /tmp/strings053; exit 1; }
( cd $W/emulation && bash tests/run_strhooks_selftest.sh ) > /tmp/strhooks053 2>&1 || { echo "strhooks selftest RED"; cat /tmp/strhooks053; exit 1; }
grep -q "teeth confirmed" /tmp/strhooks053 || { echo "strhooks teeth NOT confirmed"; exit 1; }
( cd $W/emulation && bash tests/run_vform_selftest.sh ) > /tmp/vform053 2>&1 || { echo "vform selftest RED"; cat /tmp/vform053; exit 1; }
grep -q "teeth confirmed" /tmp/vform053 || { echo "vform teeth NOT confirmed"; exit 1; }
JOBS=${JOBS:-3}
T0=$(date +%s)
...
sed 's|("127.0.0.1", 8781)|("127.0.0.1", int(__import__("os").environ.get("QUEST_PORT", "8781")))|' \
  $W/docs/Project13/drive.py > /tmp/drive053.py

cat > /tmp/blk053.py <<'PYEOF'
import re, sys
...
leg(){ local tag=$1 mode=$2 drv=$3 port=$4 k=$5 floor=$6 want=$7 cfg=$8; shift 8
  local R=/tmp/run053-$tag; rm -rf $R; mkdir -p $R; cp -r $ROOT/QUEST $R/QUEST; cd $R
  ...
  local blk bstat=OK; blk=$(python3 /tmp/blk053.py $R/trace $k $floor) || bstat=FAIL
  local end="clean"
...
launch play      play     /tmp/drive053.py         8895 50 1000  I.STOP         book "${STR[@]}"
launch play-st   play     /tmp/drive053.py         8899 50 1000  I.STOP         stock "${STR[@]}"
launch fo        failopen /tmp/drive053.py         8891 50 1000  clean,I.STOP   book "${STR[@]}"  QUEST_FAIL_OPEN=USER_DATA_FILE
launch m         m        /tmp/drive053.py         8892 50 1000  I.STOP         book "${STR[@]}"
launch inj       play     /tmp/drive053.py         8893 50 1000  FATAL          book "${STR[@]}"  QUEST_INJECT=7016A896:-1:0x2006
launch abort     m        /tmp/drive053.py         8894 50 10    WORLD-ABORT    book "${STR[@]}" QUEST_TERMINAL=7016871D:ABORT
launch inj3      m        /tmp/drive053.py         8896 50 1000  clean,I.STOP   book "${STR[@]}"  QUEST_INJECT=70176AA7:-1:0x2006:RESUME
launch k1fo      failopen /tmp/drive053.py         8897 1  1000  clean,I.STOP   book "${STR[@]}"  QUEST_FAIL_OPEN=USER_DATA_FILE
launch derr      failopen /tmp/drive053.py         8904 1  1000  WORLD-ABORT    book "${STR[@]}"  QUEST_FAIL_OPEN=USER_DATA_FILE QUEST_POKE=7015C48B:0:11
launch derr-emu  failopen /tmp/drive053.py         8905 1  1000  WORLD-ABORT    emu "${STR[@]}"   QUEST_FAIL_OPEN=USER_DATA_FILE QUEST_POKE=7015C48B:0:11
launch fo-st     failopen /tmp/drive053.py         8898 50 1000  clean,I.STOP   stock "${STR[@]}" QUEST_FAIL_OPEN=USER_DATA_FILE
launch fo-emu    failopen /tmp/drive053.py         8900 50 1000  clean,I.STOP   emu "${STR[@]}"   QUEST_FAIL_OPEN=USER_DATA_FILE
launch m-emu     m        /tmp/drive053.py         8901 50 1000  I.STOP         emu "${STR[@]}"
launch inj-emu   play     /tmp/drive053.py         8902 50 1000  FATAL          emu "${STR[@]}"   QUEST_INJECT=7016A896:-1:0x2006
launch abort-emu m        /tmp/drive053.py         8903 50 10    WORLD-ABORT    emu "${STR[@]}"   QUEST_TERMINAL=7016871D:ABORT
launch forced    failopen /tmp/drive053.py         8907 1  10    DIVERGENCE     book "${STR[@]}" QUEST_FAIL_OPEN=USER_DATA_FILE QUEST_POKE=7015C48B:2:0x75000000:CLONE
wait || true
...
echo "== Task 053 P49 STAGE B: the play-driver fix (drive_patient retired; play legs require I.STOP) (JOBS=$JOBS) ==" | tee -a $RES/verdicts.txt
...
grep -h "^strings.*QUEST1 claim" /tmp/run053-*/trace 2>/dev/null | sed 's/.*block=\([0-9A-F]*\).*/\1/' | sort | uniq -c | sort -rn | tee -a $RES/verdicts.txt
...
tail -1 /tmp/selftest053 | tee -a $RES/verdicts.txt; tail -1 /tmp/strings053 | tee -a $RES/verdicts.txt; tail -1 /tmp/strhooks053 | tee -a $RES/verdicts.txt; tail -1 /tmp/vform053 | tee -a $RES/verdicts.txt
...
[ "$FAILS" = "0" ] || { echo "TASK 053 RED ($FAILS fails)"; exit 1; }
echo "TASK 053 GREEN"
