...
SRC=$(bin/task_source.sh main 049-region-bounds); W=$SRC/Work
RES=$ROOT/results/049-region-bounds; mkdir -p $RES
...
run() { # $1=capture env dumpfile
  local dump=$1; shift
  ( cd $W && QUEST_CAPTURE=7016DF39 QUEST_CAPTURE_DEST="$1" QUEST_CAPTURE_LEN="$2" \
      QUEST_CAPTURE_WINDOWS="$3" \
      timeout 120 $EMU -silent QUEST QUEST_SERVER @QUEST @QUEST > $RES/game.log 2>$RES/cap.txt ) &
  local emupid=$!
  sleep 6
...
run $RES/objptr.txt "0x70000212" 2 ""
...
echo "TASK 049 DONE" | tee -a $RES/verdicts.txt
