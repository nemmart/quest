#!/bin/bash
# Task 031 — P22 Gen-6.0 recalibration gate, RERUN of task 029.
#
# Task 029/030 history: 029's GREEN was invalid (environmental, recorded in the P22 report):
...
sed 's|("127.0.0.1", 8781)|("127.0.0.1", int(__import__("os").environ.get("QUEST_PORT", "8781")))|' \
  $W/docs/Project13/drive.py > /tmp/drive031.py
sed 's|("127.0.0.1", 8781)|("127.0.0.1", int(__import__("os").environ.get("QUEST_PORT", "8781")))|' \
  $W/docs/Project14/drive_patient.py > /tmp/drive_patient031.py

cat > /tmp/blk031.py <<'PYEOF'
import re, sys
...
leg(){ local tag=$1 mode=$2 drv=$3 port=$4 k=$5 floor=$6 want=$7; shift 7
  local R=/tmp/run031-$tag; rm -rf $R; mkdir -p $R; cp -r $ROOT/QUEST $R/QUEST; cd $R
  ...
  local blk bstat=OK; blk=$(python3 /tmp/blk031.py $R/trace $k $floor) || bstat=FAIL
  local end="clean"
...
leg fo     failopen /tmp/drive031.py         8791 50 1000  clean,I.STOP     QUEST_FAIL_OPEN=USER_DATA_FILE
leg m      m        /tmp/drive031.py         8792 50 1000  I.STOP
leg inj    play     /tmp/drive031.py         8793 50 1000  FATAL            QUEST_INJECT=7016A896:-1:0x2006
leg abort  m        /tmp/drive031.py         8794 50 10    WORLD-ABORT      QUEST_TERMINAL=7016871D:ABORT
leg play   play     /tmp/drive_patient031.py 8795 50 1000  clean,I.STOP
leg inj2   login    /tmp/drive031.py         8796 50 200   FATAL            QUEST_INJECT=70176AA7:-1:0x2006
leg inj3   m        /tmp/drive031.py         8797 50 1000  clean,I.STOP     QUEST_INJECT=70176AA7:-1:0x2006:RESUME
leg k1fo   failopen /tmp/drive031.py         8798 1  1000  clean,I.STOP     QUEST_FAIL_OPEN=USER_DATA_FILE
