#!/bin/bash
# Task 033 — F6 terminal legs (user ruling Aug 29: option c, both halves).
#
# Tree: checks out origin/main -- Work Disassembled (P23+P24+integration
# + the F6 loader fix: IRExec drops the IR block containing a mid-block
# QUEST_INJECT/QUEST_TERMINAL pc, so both engines observe the armed pc).
# Legs: the two task-032 reds re-run under book IR (expect green with
# one dropped block each), plus all-emulated variants of the same two
# legs as the standing isolation check — they guard the terminal
# machinery independent of whatever the IR layer is doing.
...
set -eu
exec 9>/tmp/quest-f6-battery.lock
...
RES=$ROOT/results/033-f6-terminal-legs; mkdir -p $RES
: > $RES/verdicts.txt
for prt in 8791 8792 8793 8794; do fuser -k -TERM $prt/tcp 2>/dev/null || true; done; sleep 2

sed 's|("127.0.0.1", 8781)|("127.0.0.1", int(__import__("os").environ.get("QUEST_PORT", "8781")))|' \
  $W/docs/Project13/drive.py > /tmp/drive033.py
sed 's|("127.0.0.1", 8781)|("127.0.0.1", int(__import__("os").environ.get("QUEST_PORT", "8781")))|' \
  $W/docs/Project14/drive_patient.py > /tmp/drive_patient032.py

cat > /tmp/blk033.py <<'PYEOF'
import re, sys
...
leg(){ local tag=$1 mode=$2 drv=$3 port=$4 k=$5 floor=$6 want=$7 cfg=$8; shift 8
  local R=/tmp/run033-$tag; rm -rf $R; mkdir -p $R; cp -r $ROOT/QUEST $R/QUEST; cd $R
  ...
  local blk bstat=OK; blk=$(python3 /tmp/blk033.py $R/trace $k $floor) || bstat=FAIL
  local end="clean"
...
leg inj       play /tmp/drive033.py 8791 50 1000  FATAL       book QUEST_INJECT=7016A896:-1:0x2006
leg abort     m    /tmp/drive033.py 8792 50 10    WORLD-ABORT book QUEST_TERMINAL=7016871D:ABORT
leg inj-emu   play /tmp/drive033.py 8793 50 1000  FATAL       emu  QUEST_INJECT=7016A896:-1:0x2006
leg abort-emu m    /tmp/drive033.py 8794 50 10    WORLD-ABORT emu  QUEST_TERMINAL=7016871D:ABORT

echo "== Task 033 F6 terminal-leg verdicts ==" | tee -a $RES/verdicts.txt
for tag in inj abort; do
  ...
  [ "$d" = "1" ] || { echo "F6 drop check FAILED for $tag"; FAILS=$((FAILS+1)); }
done
for tag in inj-emu abort-emu; do
  d=$(grep -c 'mid-block — dropping' $RES/$tag.err || true)
  echo "$tag: armed-pc block drops=$d (want 0 — no IR loaded)" | tee -a $RES/verdicts.txt
  [ "$d" = "0" ] || { echo "unexpected drop in emu leg $tag"; FAILS=$((FAILS+1)); }
done
cat $RES/verdicts.txt
[ "$FAILS" = "0" ] || { echo "F6 BATTERY RED ($FAILS legs)"; exit 1; }
echo "F6 BATTERY GREEN"
