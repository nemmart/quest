#!/bin/bash
# Task 054 — P49 STAGE B, RE-RUN. 053's slot is burned (ATTEMPTS=3, the cap) after the runner
# started a second attempt while the first was still running and `rm -rf results/$name` wiped
# attempt 1's data; the integrator restored it from git (results/053-p49-stage-b/RESTORED.md).
#
# 053 attempt 1 was NOT a failure of the driver fix. It executed 13,910,350 pairs against task
# 052's 2,097,562 — ~6.6x more of the game — with blk_mismatch=0 and div=0 on both play legs.
# It failed on exactly one thing, which its own new instrumentation caught and named:
#
#   ##### DRIVER: CEILING HIT after 120s waiting for: exit LIST_PLAYERS
#
# Cause, from the play.session this task started keeping (docs/Project49/q003): the auto-move
# wait used QUIESCENCE, and quiescence cannot decide an auto-move. The game prints
# "-> Waiting for your turn" and then RE-EMITS THE PROMPT while still blocked on the server
# tick, so "is a prompt the most recent thing" reads as idle mid-move. Under -lockstep the gap
# to the next turn is long, so the wait returned after 2 of 3 turns; all five menu keys were
# then delivered mid-auto-move and the ceiling surfaced at the LAST step, naming the wrong one.
# ZERO screens were reached — the same failure mode this project exists to fix, in new clothes.
#
# Fix in drive.py: the auto-move is waited on by COUNTING TURNS off the constant direction echo
# ("Move to the " — one echo plus one per turn), with a stall timeout so an auto-move cut short
# by a random encounter is reported loudly and continues instead of burning the ceiling.
# Per-step ceilings raised 120 -> 300 s for lockstep pacing.
#
# Task 053 — P49 STAGE B: the play-driver fix, on branch p49-playdriver.
# Task 051 (task 052 with the three stale P33 expectations refreshed) with the play legs
# re-pointed at the REWRITTEN docs/Project13/drive.py. 051 is GREEN and DONE, so every static
# expectation below is already known-good; the only moving part here is the driver.
#
# What changed in the driver (docs/Project49/q001-plan-gate.md section 1). All three bugs were
# found by reading a play session log for the first time — the artifact FINDINGS_SUMMARY
# section 2 told everyone to read has never existed in the repo, because leg() copies
# session.log only on FAILURE and the play legs were scored OK:
#   1. the ESC after `D` landed on the COMMAND PROMPT and detached. DISPLAY_MAGIC prints
#      "All spells ready!" and returns; it has no screen to escape. The session died there and
#      L/H were never sent. There is no ESC after D now.
#   2. `L` opens a chooser ("Hit (P) for players ..."); the old driver answered it with ESC, so
#      LIST_PLAYERS rendered its menu and the listing body never ran. `P` is sent now.
#   3. fixed waits replaced by the game's own handshake. Measured: sitting at the command
#      prompt the game emits NOTHING (100 s sampled, 0 bytes), so quiescence is sound; the
#      turn-wait test is by ORDER (a prompt after the last "Waiting for your turn"), not by
#      presence, or a spent marker wedges the wait forever.
#
# Ruling 7a: docs/Project14/drive_patient.py is RETIRED. It was a one-off `play` fork that had
# quietly become the thing running in the battery's play leg, and its budget was ~498 s inside
# a `timeout 480` — so in 047b/050/052 the play leg's menu keys were NEVER SENT AT ALL.
# Ruling 7b: play legs get `timeout 1200`, and driver stdout is captured to $RES/<tag>.driver
# so a ceiling hit is loud instead of looking like a clean end.
# Ruling 2: play/play-st now REQUIRE I.STOP. `end=clean` was a silent kill.
#
# NEW TEETH (all new in 053):
#   * ceiling hits across every leg, want 0 — the driver reports its own timing failures
#   * the four screens the fix is supposed to reach, asserted BY NAME out of the play session
#     logs, which are now kept unconditionally rather than only on failure
#   * HELP chain block 7016D5E3 live in play AND play-st — informational in 047b/050/051/052,
#     a real check here
#   * OBSERVE / DISPLAY_MAGIC / LIST_PLAYERS / HELP entry blocks live in the union of IR legs
#
# Bar: 16/16 legs, 0 div on every non-forced leg, play+play-st end I.STOP, 0 ceiling hits, the
# four screens present, 7016D5E3 live in both play legs, every 051 static line still green.
set -eu
exec 9>/tmp/quest-parallel-battery.lock
flock -n 9 || { echo "another battery attempt is still running; refusing overlap"; exit 1; }
cd "$(dirname "$0")/.."
ROOT=$(pwd)
SRC=$(bin/task_source.sh p49-playdriver 054-p49-stage-b); W=$SRC/Work
...
RES=$ROOT/results/054-p49-stage-b; mkdir -p $RES
...
( cd $W/emulation && bash tests/run_helpers_selftest.sh ) > /tmp/selftest054 2>&1 || { echo "helpers selftest RED"; cat /tmp/selftest054; exit 1; }
( cd $W/emulation && bash tests/run_strings_selftest.sh ) > /tmp/strings054 2>&1 || { echo "strings selftest RED"; cat /tmp/strings054; exit 1; }
( cd $W/emulation && bash tests/run_strhooks_selftest.sh ) > /tmp/strhooks054 2>&1 || { echo "strhooks selftest RED"; cat /tmp/strhooks054; exit 1; }
grep -q "teeth confirmed" /tmp/strhooks054 || { echo "strhooks teeth NOT confirmed"; exit 1; }
( cd $W/emulation && bash tests/run_vform_selftest.sh ) > /tmp/vform054 2>&1 || { echo "vform selftest RED"; cat /tmp/vform054; exit 1; }
grep -q "teeth confirmed" /tmp/vform054 || { echo "vform teeth NOT confirmed"; exit 1; }
JOBS=${JOBS:-3}
T0=$(date +%s)
...
sed 's|("127.0.0.1", 8781)|("127.0.0.1", int(__import__("os").environ.get("QUEST_PORT", "8781")))|' \
  $W/docs/Project13/drive.py > /tmp/drive054.py

cat > /tmp/blk054.py <<'PYEOF'
import re, sys
...
leg(){ local tag=$1 mode=$2 drv=$3 port=$4 k=$5 floor=$6 want=$7 cfg=$8; shift 8
  local R=/tmp/run054-$tag; rm -rf $R; mkdir -p $R; cp -r $ROOT/QUEST $R/QUEST; cd $R
  ...
  local EP=$!; sleep 8
  local tmo=480; if [ "$mode" = "play" ]; then tmo=1200; fi   # ruling 7b: the patient wait needs room
  QUEST_PORT=$port QUEST_DRIVE_QUIET_SLICES=${QSLICES:-5} \
      timeout $tmo python3 $drv $mode $R/session.log > $R/driver.out 2>&1 || true
  ...
  local blk bstat=OK; blk=$(python3 /tmp/blk054.py $R/trace $k $floor) || bstat=FAIL
  local end="clean"
  ...
  cp $R/driver.out $RES/$tag.driver 2>/dev/null || true
  case $tag in play|play-st) cp $R/session.log $RES/$tag.session 2>/dev/null || true;; esac
...
launch play      play     /tmp/drive054.py         8895 50 1000  I.STOP         book "${STR[@]}"
launch play-st   play     /tmp/drive054.py         8899 50 1000  I.STOP         stock "${STR[@]}"
launch fo        failopen /tmp/drive054.py         8891 50 1000  clean,I.STOP   book "${STR[@]}"  QUEST_FAIL_OPEN=USER_DATA_FILE
launch m         m        /tmp/drive054.py         8892 50 1000  I.STOP         book "${STR[@]}"
launch inj       play     /tmp/drive054.py         8893 50 1000  FATAL          book "${STR[@]}"  QUEST_INJECT=7016A896:-1:0x2006
launch abort     m        /tmp/drive054.py         8894 50 10    WORLD-ABORT    book "${STR[@]}" QUEST_TERMINAL=7016871D:ABORT
launch inj3      m        /tmp/drive054.py         8896 50 1000  clean,I.STOP   book "${STR[@]}"  QUEST_INJECT=70176AA7:-1:0x2006:RESUME
launch k1fo      failopen /tmp/drive054.py         8897 1  1000  clean,I.STOP   book "${STR[@]}"  QUEST_FAIL_OPEN=USER_DATA_FILE
launch derr      failopen /tmp/drive054.py         8904 1  1000  WORLD-ABORT    book "${STR[@]}"  QUEST_FAIL_OPEN=USER_DATA_FILE QUEST_POKE=7015C48B:0:11
launch derr-emu  failopen /tmp/drive054.py         8905 1  1000  WORLD-ABORT    emu "${STR[@]}"   QUEST_FAIL_OPEN=USER_DATA_FILE QUEST_POKE=7015C48B:0:11
launch fo-st     failopen /tmp/drive054.py         8898 50 1000  clean,I.STOP   stock "${STR[@]}" QUEST_FAIL_OPEN=USER_DATA_FILE
launch fo-emu    failopen /tmp/drive054.py         8900 50 1000  clean,I.STOP   emu "${STR[@]}"   QUEST_FAIL_OPEN=USER_DATA_FILE
launch m-emu     m        /tmp/drive054.py         8901 50 1000  I.STOP         emu "${STR[@]}"
launch inj-emu   play     /tmp/drive054.py         8902 50 1000  FATAL          emu "${STR[@]}"   QUEST_INJECT=7016A896:-1:0x2006
launch abort-emu m        /tmp/drive054.py         8903 50 10    WORLD-ABORT    emu "${STR[@]}"   QUEST_TERMINAL=7016871D:ABORT
launch forced    failopen /tmp/drive054.py         8907 1  10    DIVERGENCE     book "${STR[@]}" QUEST_FAIL_OPEN=USER_DATA_FILE QUEST_POKE=7015C48B:2:0x75000000:CLONE
wait || true
...
echo "== Task 054 P49 STAGE B (re-run): the play-driver fix (drive_patient retired; play legs require I.STOP) (JOBS=$JOBS) ==" | tee -a $RES/verdicts.txt
...
grep -h "^strings.*QUEST1 claim" /tmp/run054-*/trace 2>/dev/null | sed 's/.*block=\([0-9A-F]*\).*/\1/' | sort | uniq -c | sort -rn | tee -a $RES/verdicts.txt
...
tail -1 /tmp/selftest054 | tee -a $RES/verdicts.txt; tail -1 /tmp/strings054 | tee -a $RES/verdicts.txt; tail -1 /tmp/strhooks054 | tee -a $RES/verdicts.txt; tail -1 /tmp/vform054 | tee -a $RES/verdicts.txt
...
echo "-- P49 STAGE B: the driver fix, scored (q001 section 1) --" | tee -a $RES/verdicts.txt
B=0   # fails belonging to Stage B itself, scored apart from the inherited battery
CEIL=$(grep -h "CEILING HIT" $RES/*.driver 2>/dev/null | wc -l)
echo "driver ceiling hits across all legs=$CEIL (want 0; each one names the step it gave up on)" | tee -a $RES/verdicts.txt
[ "$CEIL" = "0" ] || { FAILS=$((FAILS+1)); B=$((B+1)); }
grep -h "CEILING HIT" $RES/*.driver 2>/dev/null | sed 's/^/  /' | tee -a $RES/verdicts.txt
# The four screens the fix exists to reach, asserted by name out of the kept session logs.
# "All spells ready" is DISPLAY_MAGIC returning to the prompt — the key whose stray ESC was
# quitting the game. "Hit (P) for players" plus a table header is LIST_PLAYERS' BODY, which no
# battery has ever executed. "Terrain symbols" is HELP topic 1 (the P32 scratch chain).
SCRFAIL=0
for tag in play play-st; do
  L=$RES/$tag.session
  for pat in "Observe item" "All spells ready" "Hit (P) for players" "Terrain symbols are"; do
    # NB: `grep -c || echo 0` would print "0\n0" on a miss and then compare != "0" and PASS.
    n=$(grep -ac "$pat" $L 2>/dev/null || true); n=${n:-0}
    printf "%s: %-22s %s\n" "$tag" "$pat" "$n" | tee -a $RES/verdicts.txt
    [ "$n" -ge 1 ] 2>/dev/null || SCRFAIL=$((SCRFAIL+1))
  done
done
echo "screens missing from the play session logs=$SCRFAIL (want 0)" | tee -a $RES/verdicts.txt
[ "$SCRFAIL" = "0" ] || { FAILS=$((FAILS+1)); B=$((B+1)); }
# 7016D5E3: HELP's two-stage 164 || 150 -> 314 scratch chain. INFORMATIONAL since 047b because
# the keys never landed; a real check now.
HC=0
for tag in play play-st; do
  c=$(grep -c "first execution of block 7016D5E3" $RES/$tag.err 2>/dev/null || true); c=${c:-0}
  echo "$tag: HELP chain block 7016D5E3 first-executed=$c (want >=1)" | tee -a $RES/verdicts.txt
  [ "$c" -ge 1 ] 2>/dev/null || HC=$((HC+1))
done
[ "$HC" = "0" ] || { FAILS=$((FAILS+1)); B=$((B+1)); }
# Entry blocks of the four nodes that were 0/N in every battery to date (P47 Coverage.md).
echo "-- P49 STAGE B: nodes that were 0-coverage before the fix (entry blocks from quest.addrbook; union of IR legs) --" | tee -a $RES/verdicts.txt
for nb in "OBSERVE:70172CE1" "DISPLAY_MAGIC:70166487" "LIST_PLAYERS:7016EB87" "HELP:7016D55F"; do
  nm=${nb%%:*}; b=${nb##*:}
  c=$(grep -h "first execution of block $b" $RES/*.err 2>/dev/null | wc -l)
  printf "%-14s %s live_in_legs=%s\n" "$nm" "$b" "$c" | tee -a $RES/verdicts.txt
done
echo "stage_b_fails=$B (want 0)" | tee -a $RES/verdicts.txt
echo "-- P49 STAGE A verdict (the three refreshed lines, inherited from 051) --" | tee -a $RES/verdicts.txt
echo "stage_a_refreshed_lines_failing=$A3 (want 0)  other_fails=$((FAILS-A3))" | tee -a $RES/verdicts.txt
if [ "$A3" = "0" ] && [ "$FAILS" != "0" ]; then
  echo "STAGE A lines still GREEN; battery RED for $((FAILS-A3)) other fail(s) — read the leg table and the Stage B section above" | tee -a $RES/verdicts.txt
fi
echo "wall_clock_seconds=$(( $(date +%s) - T0 ))" | tee -a $RES/verdicts.txt
cat $RES/verdicts.txt
[ "$FAILS" = "0" ] || { echo "TASK 054 RED ($FAILS fails)"; exit 1; }
echo "TASK 054 GREEN"
