...
JOBS=${JOBS:-3}
T0=$(date +%s)
for prt in $(seq 8871 8887); do fuser -k -TERM $prt/tcp 2>/dev/null || true; done; sleep 2

sed 's|("127.0.0.1", 8781)|("127.0.0.1", int(__import__("os").environ.get("QUEST_PORT", "8781")))|' \
  $W/docs/Project13/drive.py > /tmp/drive047.py
sed 's|("127.0.0.1", 8781)|("127.0.0.1", int(__import__("os").environ.get("QUEST_PORT", "8781")))|' \
  $W/docs/Project14/drive_patient.py > /tmp/drive_patient047.py

cat > /tmp/blk047.py <<'PYEOF'
import re, sys
...
leg(){ local tag=$1 mode=$2 drv=$3 port=$4 k=$5 floor=$6 want=$7 cfg=$8; shift 8
  local R=/tmp/run047-$tag; rm -rf $R; mkdir -p $R; cp -r $ROOT/QUEST $R/QUEST; cd $R
  ...
  local blk bstat=OK; blk=$(python3 /tmp/blk047.py $R/trace $k $floor) || bstat=FAIL
  local end="clean"
...
launch play      play     /tmp/drive_patient047.py 8875 50 1000  clean,I.STOP   book "${STR[@]}"
launch play-st   play     /tmp/drive047.py         8879 50 1000  clean,I.STOP   stock "${STR[@]}"
launch fo        failopen /tmp/drive047.py         8871 50 1000  clean,I.STOP   book "${STR[@]}"  QUEST_FAIL_OPEN=USER_DATA_FILE
launch m         m        /tmp/drive047.py         8872 50 1000  I.STOP         book "${STR[@]}"
launch inj       play     /tmp/drive047.py         8873 50 1000  FATAL          book "${STR[@]}"  QUEST_INJECT=7016A896:-1:0x2006
launch abort     m        /tmp/drive047.py         8874 50 10    WORLD-ABORT    book "${STR[@]}" QUEST_TERMINAL=7016871D:ABORT
launch inj3      m        /tmp/drive047.py         8876 50 1000  clean,I.STOP   book "${STR[@]}"  QUEST_INJECT=70176AA7:-1:0x2006:RESUME
launch k1fo      failopen /tmp/drive047.py         8877 1  1000  clean,I.STOP   book "${STR[@]}"  QUEST_FAIL_OPEN=USER_DATA_FILE
launch derr      failopen /tmp/drive047.py         8884 1  1000  WORLD-ABORT    book "${STR[@]}"  QUEST_FAIL_OPEN=USER_DATA_FILE QUEST_POKE=7015C48B:0:11
launch derr-emu  failopen /tmp/drive047.py         8885 1  1000  WORLD-ABORT    emu "${STR[@]}"   QUEST_FAIL_OPEN=USER_DATA_FILE QUEST_POKE=7015C48B:0:11
launch fo-st     failopen /tmp/drive047.py         8878 50 1000  clean,I.STOP   stock "${STR[@]}" QUEST_FAIL_OPEN=USER_DATA_FILE
launch fo-emu    failopen /tmp/drive047.py         8880 50 1000  clean,I.STOP   emu "${STR[@]}"   QUEST_FAIL_OPEN=USER_DATA_FILE
launch m-emu     m        /tmp/drive047.py         8881 50 1000  I.STOP         emu "${STR[@]}"
launch inj-emu   play     /tmp/drive047.py         8882 50 1000  FATAL          emu "${STR[@]}"   QUEST_INJECT=7016A896:-1:0x2006
launch abort-emu m        /tmp/drive047.py         8883 50 10    WORLD-ABORT    emu "${STR[@]}"   QUEST_TERMINAL=7016871D:ABORT
launch forced    failopen /tmp/drive047.py         8887 1  10    DIVERGENCE     book "${STR[@]}" QUEST_FAIL_OPEN=USER_DATA_FILE QUEST_POKE=7015C48B:2:0x75000000:CLONE
wait || true
...
echo "== Task 047 P33-B arena twins battery (checker ON, QUEST_ARENA) (JOBS=$JOBS) ==" | tee -a $RES/verdicts.txt
...
grep -h "^strings.*QUEST1 claim" /tmp/run047-*/trace 2>/dev/null | sed 's/.*block=\([0-9A-F]*\).*/\1/' | sort | uniq -c | sort -rn | tee -a $RES/verdicts.txt
...
tail -1 /tmp/selftest047 | tee -a $RES/verdicts.txt; tail -1 /tmp/strings047 | tee -a $RES/verdicts.txt; tail -1 /tmp/strhooks047 | tee -a $RES/verdicts.txt
