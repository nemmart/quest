#!/bin/bash
# Task 041 — P28 rt_call decoration (docs/Project28/{PROMPT,Census,RTConventions,REPORT}.md).
...
cd "$(dirname "$0")/.."
ROOT=$(pwd)
SRC=$(bin/task_source.sh p28-rt-call 041-p28-rt-call); W=$SRC/Work
...
RES=$ROOT/results/041-p28-rt-call; mkdir -p $RES
JOBS=${JOBS:-3}
T0=$(date +%s)
...
sed 's|("127.0.0.1", 8781)|("127.0.0.1", int(__import__("os").environ.get("QUEST_PORT", "8781")))|' \
  $W/docs/Project13/drive.py > /tmp/drive041.py
sed 's|("127.0.0.1", 8781)|("127.0.0.1", int(__import__("os").environ.get("QUEST_PORT", "8781")))|' \
  $W/docs/Project14/drive_patient.py > /tmp/drive_patient034.py

cat > /tmp/blk041.py <<'PYEOF'
import re, sys
...
leg(){ local tag=$1 mode=$2 drv=$3 port=$4 k=$5 floor=$6 want=$7 cfg=$8; shift 8
  local R=/tmp/run041-$tag; rm -rf $R; mkdir -p $R; cp -r $ROOT/QUEST $R/QUEST; cd $R
  ...
  local blk bstat=OK; blk=$(python3 /tmp/blk041.py $R/trace $k $floor) || bstat=FAIL
  local end="clean"
...
launch play-st   play     /tmp/drive041.py         8839 50 1000  clean,I.STOP   stock
launch fo        failopen /tmp/drive041.py         8831 50 1000  clean,I.STOP   book  QUEST_FAIL_OPEN=USER_DATA_FILE
launch m         m        /tmp/drive041.py         8832 50 1000  I.STOP         book
launch inj       play     /tmp/drive041.py         8833 50 1000  FATAL          book  QUEST_INJECT=7016A896:-1:0x2006
launch abort     m        /tmp/drive041.py         8834 50 10    WORLD-ABORT    book  QUEST_TERMINAL=7016871D:ABORT
launch inj3      m        /tmp/drive041.py         8836 50 1000  clean,I.STOP   book  QUEST_INJECT=70176AA7:-1:0x2006:RESUME
launch k1fo      failopen /tmp/drive041.py         8837 1  1000  clean,I.STOP   book  QUEST_FAIL_OPEN=USER_DATA_FILE
launch derr      failopen /tmp/drive041.py         8844 1  1000  IR-ASSERT      book  QUEST_FAIL_OPEN=USER_DATA_FILE QUEST_POKE=7015C48B:0:11
launch derr-emu  failopen /tmp/drive041.py         8845 1  1000  WORLD-ABORT    emu   QUEST_FAIL_OPEN=USER_DATA_FILE QUEST_POKE=7015C48B:0:11
launch fo-st     failopen /tmp/drive041.py         8838 50 1000  clean,I.STOP   stock QUEST_FAIL_OPEN=USER_DATA_FILE
launch fo-emu    failopen /tmp/drive041.py         8840 50 1000  clean,I.STOP   emu   QUEST_FAIL_OPEN=USER_DATA_FILE
launch m-emu     m        /tmp/drive041.py         8841 50 1000  I.STOP         emu
launch inj-emu   play     /tmp/drive041.py         8842 50 1000  FATAL          emu   QUEST_INJECT=7016A896:-1:0x2006
launch abort-emu m        /tmp/drive041.py         8843 50 10    WORLD-ABORT    emu   QUEST_TERMINAL=7016871D:ABORT
wait || true
...
echo "== Task 041 P28 rt_call battery (JOBS=$JOBS) ==" | tee -a $RES/verdicts.txt
...
LE=$(grep -c ' emitted ' $LED || true); LR=$(grep -c ' REFUSED ' $LED || true)
echo "rt_sites ledger: emitted=$LE refused=$LR (want 987/0)" | tee -a $RES/verdicts.txt
[ "$LE" = "987" ] && [ "$LR" = "0" ] || FAILS=$((FAILS+1))
LEFT=$(grep -cE '^  @[0-9A-F]+ (LNDO|LDSP|[XL]PEF |WPSH 0,0|(MOV|ADD|SUB|COM|NEG|ADC|INC|AND)(\.[ZOC]?[LRS]?)? )' $IRB || true)
