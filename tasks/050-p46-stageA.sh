#!/bin/bash
# Task 050 — P46 Stage A: the arena twins respelled t@<block>.<k> -> s@<block>.<k> (ir 7),
# docs/Project46/{PROMPT,q001-plan-gate,a001-plan-gate}.md. Branch p46-ir7. The token rename
# was done by REGENERATION through the P33-B chain (strhooks.py -> arena.py -> string_sites.py
# --p33 -> lower.py), proven token-only in docs/Project46/evidence/stageA_rename.txt (236 tokens
# on 198 statements per artifact; the only other differences are the `ir 7` line and the
# strings33/arena provenance sha lines). This task is 047b's battery VERBATIM except:
#   - source branch p46-ir7; all `t@` expectations read `s@`; ir header wants `ir 7`;
#   - a new verdict: the artifacts contain ZERO `t@` tokens and 236 `s@` tokens each.
# Everything else (legs, ports, K, floors, endpoints, verdict lines, bar 15/15 + forced) is 047b's,
# because Stage A must change NOTHING on the strict surface but the spelling.
# Note on 047b: its result dir is FAILED (exit=1) — the P33-C report records why; this task's
# bar is 047b's bar, and any leg that was red there for a known non-rename reason is reported,
# not masked (a red run is a STOP-and-report per METHOD §15).
set -eu
exec 9>/tmp/quest-parallel-battery.lock
flock -n 9 || { echo "another battery attempt is still running; refusing overlap"; exit 1; }
cd "$(dirname "$0")/.."
ROOT=$(pwd)
SRC=$(bin/task_source.sh p46-ir7 050-p46-stageA); W=$SRC/Work
...
RES=$ROOT/results/050-p46-stageA; mkdir -p $RES
...
( cd $W/emulation && bash tests/run_helpers_selftest.sh ) > /tmp/selftest050 2>&1 || { echo "helpers selftest RED"; cat /tmp/selftest050; exit 1; }
( cd $W/emulation && bash tests/run_strings_selftest.sh ) > /tmp/strings050 2>&1 || { echo "strings selftest RED"; cat /tmp/strings050; exit 1; }
( cd $W/emulation && bash tests/run_strhooks_selftest.sh ) > /tmp/strhooks050 2>&1 || { echo "strhooks selftest RED"; cat /tmp/strhooks050; exit 1; }
grep -q "teeth confirmed" /tmp/strhooks050 || { echo "strhooks teeth NOT confirmed"; exit 1; }
JOBS=${JOBS:-3}
T0=$(date +%s)
...
sed 's|("127.0.0.1", 8781)|("127.0.0.1", int(__import__("os").environ.get("QUEST_PORT", "8781")))|' \
  $W/docs/Project13/drive.py > /tmp/drive050.py
sed 's|("127.0.0.1", 8781)|("127.0.0.1", int(__import__("os").environ.get("QUEST_PORT", "8781")))|' \
  $W/docs/Project14/drive_patient.py > /tmp/drive_patient050.py

cat > /tmp/blk050.py <<'PYEOF'
import re, sys
...
leg(){ local tag=$1 mode=$2 drv=$3 port=$4 k=$5 floor=$6 want=$7 cfg=$8; shift 8
  local R=/tmp/run050-$tag; rm -rf $R; mkdir -p $R; cp -r $ROOT/QUEST $R/QUEST; cd $R
  ...
  local blk bstat=OK; blk=$(python3 /tmp/blk050.py $R/trace $k $floor) || bstat=FAIL
  local end="clean"
...
launch play      play     /tmp/drive_patient050.py 8895 50 1000  clean,I.STOP   book "${STR[@]}"
launch play-st   play     /tmp/drive050.py         8899 50 1000  clean,I.STOP   stock "${STR[@]}"
launch fo        failopen /tmp/drive050.py         8891 50 1000  clean,I.STOP   book "${STR[@]}"  QUEST_FAIL_OPEN=USER_DATA_FILE
launch m         m        /tmp/drive050.py         8892 50 1000  I.STOP         book "${STR[@]}"
launch inj       play     /tmp/drive050.py         8893 50 1000  FATAL          book "${STR[@]}"  QUEST_INJECT=7016A896:-1:0x2006
launch abort     m        /tmp/drive050.py         8894 50 10    WORLD-ABORT    book "${STR[@]}" QUEST_TERMINAL=7016871D:ABORT
launch inj3      m        /tmp/drive050.py         8896 50 1000  clean,I.STOP   book "${STR[@]}"  QUEST_INJECT=70176AA7:-1:0x2006:RESUME
launch k1fo      failopen /tmp/drive050.py         8897 1  1000  clean,I.STOP   book "${STR[@]}"  QUEST_FAIL_OPEN=USER_DATA_FILE
launch derr      failopen /tmp/drive050.py         8904 1  1000  WORLD-ABORT    book "${STR[@]}"  QUEST_FAIL_OPEN=USER_DATA_FILE QUEST_POKE=7015C48B:0:11
launch derr-emu  failopen /tmp/drive050.py         8905 1  1000  WORLD-ABORT    emu "${STR[@]}"   QUEST_FAIL_OPEN=USER_DATA_FILE QUEST_POKE=7015C48B:0:11
launch fo-st     failopen /tmp/drive050.py         8898 50 1000  clean,I.STOP   stock "${STR[@]}" QUEST_FAIL_OPEN=USER_DATA_FILE
launch fo-emu    failopen /tmp/drive050.py         8900 50 1000  clean,I.STOP   emu "${STR[@]}"   QUEST_FAIL_OPEN=USER_DATA_FILE
launch m-emu     m        /tmp/drive050.py         8901 50 1000  I.STOP         emu "${STR[@]}"
launch inj-emu   play     /tmp/drive050.py         8902 50 1000  FATAL          emu "${STR[@]}"   QUEST_INJECT=7016A896:-1:0x2006
launch abort-emu m        /tmp/drive050.py         8903 50 10    WORLD-ABORT    emu "${STR[@]}"   QUEST_TERMINAL=7016871D:ABORT
launch forced    failopen /tmp/drive050.py         8907 1  10    DIVERGENCE     book "${STR[@]}" QUEST_FAIL_OPEN=USER_DATA_FILE QUEST_POKE=7015C48B:2:0x75000000:CLONE
wait || true
...
echo "== Task 050 P46 Stage A: t@ -> s@ rename (047b battery verbatim; checker ON) (JOBS=$JOBS) ==" | tee -a $RES/verdicts.txt
...
grep -h "^strings.*QUEST1 claim" /tmp/run050-*/trace 2>/dev/null | sed 's/.*block=\([0-9A-F]*\).*/\1/' | sort | uniq -c | sort -rn | tee -a $RES/verdicts.txt
...
tail -1 /tmp/selftest050 | tee -a $RES/verdicts.txt; tail -1 /tmp/strings050 | tee -a $RES/verdicts.txt; tail -1 /tmp/strhooks050 | tee -a $RES/verdicts.txt
...
[ "$FAILS" = "0" ] || { echo "TASK 050 RED ($FAILS fails)"; exit 1; }
echo "TASK 050 GREEN"
